...
@section('content')
    <div class="container-fluid">
        ...
        <p class="text-end p-0 m-0 me-3 mt-3"><a href="{{ route('category.add') }}" class="btn btn-success">Add Category</a></p>
        <x-table title="Categories">
            @foreach ($categories as $category)
                <x-table.tr name="{{ $category->name }}">
        ...
        <div class="alert alert-warning" role="alert">
            There is no category. You should <a href="{{ route('category.add') }}" class="alert-link" >add some</a> category !!!
          </div>
        @endif
    </div>
@endsection
